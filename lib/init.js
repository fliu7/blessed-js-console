...
class JsConsole {
    ...
    start(context, customEval, helpText) {
    ...
    _createMenu() {
        ...
        blessed.box({
            ...
            let line = new EvalOutputLogLine(that.logList);
            line.set([that._helpText]);
            that.logList.pushLine(line);
            that.scrollAlong();
        });
    }
    ...
    evaluate(code) {
        ...
        const commandLine = new CommandLogLine(logList);
        commandLine.set(esc('38;5;74', code));

        // Add it to the logList
        logList.pushLine(commandLine);

        function renderLine(line) {
            logList.insertAfter(line, commandLine.index);
