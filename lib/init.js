...
const { multiply, getPath, subtract, esc, stripEsc } = require('./helpers');
...
debug(message) {
	...
	fs.appendFileSync('/tmp/janewaydebug.log', str + '\n');
}
...
getPropertyNames(target) {
	...
	if (proto) {
		return result.concat(this.getPropertyNames(proto));
	}
...
autocomplete(cmd, key) {
	...
	if (pieces.length == 1) {
		...
		last = pieces.pop();
		target = getPath(vm_context, pieces);
	...
	if (target) {
		...
		hidden = subtract(this.getPropertyNames(target), keys);

		for (i = 0; i < keys.length; i++) {
			item = keys[i];

			if (!last || item.startsWith(last)) {
				items.push(item);
			}
		}
...
Janeway = new JsConsole();

// Require all the classes
require('./class/log_list.js')(Janeway);
require('./class/log_line.js')(Janeway);
require('./class/args_log_line.js')(Janeway);
require('./class/property_log_line.js')(Janeway);
require('./class/string_log_line.js')(Janeway);
require('./class/paging_log_line.js')(Janeway);
require('./class/other_log_line.js')(Janeway);
