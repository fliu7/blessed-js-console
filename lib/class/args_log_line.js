...
class ArgsLogLine extends LogLine {
    ...
    getInlineInspect(arg, amount) {
        ...
        if (is_array) {
            ...
            if (arg.constructor.name == 'Date' && arg.getUTCDate) {
                result += formatDate(arg, this.jsConsole.config.properties.date_format);
                result += '}';
                return result;
            }
            ...
            if (this.jsConsole.config.properties.sort) {
                keys.sort();
            }
        }
        ...
        for (i = 0; i < keys.length; i++) {
            ...
            if (i >= amount) {
                result += this.jsConsole.config.strings.ellipsis;
                break;
            }
        ...
        for (i = 0; i < this.args.length; i++) {
            ...
            if (typeof arg == 'string') {
                ...
                if (arg.type == 'callerInfo') {
                    ...
                    if (this.jsConsole.args_name_map != null) {
                        name_to_show = this.jsConsole.args_name_map[arg.path] || this.logList.jsConsole.args_name_map[arg.file];
                    }
                    ...
                    if (this.jsConsole.config.caller_info.max_filename_length) {
                        if (strlen(name_to_show) > this.jsConsole.config.caller_info.max_filename_length) {
                            name_to_show = truncate(name_to_show, this.jsConsole.config.caller_info.max_filename_length, this.jsConsole.config.strings.ellipsis);
                        }
                    }
                    ...
                    if (this.jsConsole.config.caller_info.min_length > 0 && this.jsConsole.config.caller_info.min_length < 41) {
                        if (len < this.jsConsole.config.caller_info.min_length) {
                            temp = this.jsConsole.config.caller_info.min_length - len;
                            str += multiply(' ', temp);
                        }
                    }
                    ...
                    if (has_progress) {
                        ...
                        if (arg.constructor.name == 'Date' && arg.getUTCDate) {
                            str += formatDate(arg, this.jsConsole.config.properties.date_format);
                        } else {
                            str += len;
                        }
    ...
    select(absX, level) {
        ...
        if ((typeof arg == 'object' || typeof arg == 'function') && arg) {
            ...
            for (i = 0; i < keys.length; i++) {
                ...
                propLine = this.jsConsole.createPropertyLine();
            ...
            for (i = 0; i < lines.length; i++) {
                ...
                propLine = this.jsConsole.createStringLine();
